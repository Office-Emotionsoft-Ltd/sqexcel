...
- **新設10章「SQExcelの入力シートの説明」**：9章に入れ子だった「SQExcelの入力シート」グループ（概要・構造と作成機能・データ取り込み・データ出力・検索条件記入方法の5ページ）を独立した最上位グループへ昇格。「構造と作成機能」のラベルには「SQExcelの」を接頭辞として付与し「SQExcelの入力シートの構造と作成」に変更
...
### オンラインヘルプに「動作環境」ページを新設

古谷氏の依頼により、対応OS・対応.NET Version・対応DB・DBデータ型とExcelセル書式対応をまとめた新規ページ`src/content/docs/ja/docs/environment.md`を追加。サイドバー（`astro.config.mjs`）・目次（`toc.md`）とも「リリースノート」の直前に配置。

- 対応OS/対応.NET Versionの各セクション末尾に「開発・動作確認済み環境のみ記載し、それ以外はベストエフォートで順次対応」という趣旨の注記を1箇所ずつ配置。対応DBセクション末尾には「( )内は動作確認済みバージョンであり保証ではない」という趣旨の注記を配置（古谷氏指定の文言をそのまま採用）。
- 「DBのデータ型とエクセルのセル書式」セクションは、古谷氏が用意した`src/content/docs/ja/docs/images/environment/SQLDBのデータ型.xlsx`（39データ型分）をPythonで読み取り、Markdownの表（`four-column-table-equal`、既存の`comparison.md`/`data-import.md`と同じクラス）に変換して転記。表内の`@`表記は、`structure.md`で確立済みの慣習（バッククォートのコード書式）に合わせて統一（プレーンな`"@"`のままだとMarkdownのスマートクォート変換でカーブクォートに化けるため）。
- 対応DB製品名の表記・並び順は、`structure.md`で既に使われている「Microsoft SQL Server/ Oracle Database/ PostgreSQL/ MySQL/ MariaDB/ SQLite」の表記に統一（古谷氏の原案は「M/S SQLServer」等の略記だった）。
- `npm run build`（70ページ生成・エラーなし）、および生成後のHTMLを直接確認（サイドバー/目次リンクへの`base`付与、表の`@`がコード書式でレンダリングされていること）で検証済み。ブラウザ拡張未接続のため実機スクリーンショットでの最終確認は古谷氏側で実施予定。

**Why:** 「対応環境→リリースノート」という並びが、ユーザーが「まず動くか確認→次にどのバージョンで何が変わったか」を追う自然な導線になるため、リリースノートの直前という配置にした。テスト済み環境を正直に開示する方針は、後日「検証済み」表記との齟齬による信頼低下を避けるため。
**How to apply:** 今後対応DB・OS・.NETのバージョンが増減した場合は、このページの3セクション＋注記文言をあわせて更新すること。データ型対応表はソースの`SQLDBのデータ型.xlsx`が更新された場合、都度読み直して表を再生成する必要がある（自動連携はしていない）。

このコミットには、前回セッション（2026-09-05、コミット`743c2ed`）から持ち越しだった「データ取り込みの詳細」「入力シートの構造と作成」ページの文言微修正と、入力シート関連サンプルファイル（xlsx）の更新も含む。
