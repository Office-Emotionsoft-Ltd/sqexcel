---
title: データ取り込み
description: データ取り込み機能とダイアログから設定するオプションの詳細。
---

[IO操作画面](/ja/docs/features/io-operations/) で「データの取り込み」ボタンをクリックすると、データの取り込みダイアログが開きます。データが入力された入力シートを元に SQL を生成し、データベースへ反映します。
...
| No. | エリア | 内容 |
|---|---|---|
| ④-6 | INSERT文のスタイル | 項目リストや値リストの前後に付ける()の位置、及び各項目の区切りの「,」の位置を4つのパターンから選択します |
| ④-7 | UPDATE文のスタイル | SET句の位置、項目値設定句の「,」の位置、及びWHERE句内のAND/ORの位置を4つのパターンから選択します |

</div>

- それぞれのスタイルを選択したときに生成されるSQLの具体的な書式は [データ取り込みの詳細](/ja/docs/features/input-sheet/data-import/#insert文update文のスタイル) で例示しています
...
データ型ごとの入力可能な値・エラーになる値の詳しい境界は [データ取り込みと入力シート](/ja/docs/features/input-sheet/data-import/) を参照してください。
